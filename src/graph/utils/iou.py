...
def compute_3d_iou_accuracte_batch(bbox1, bbox2):
    ...
    Modified to not depend on pytorch3d.
    ...
    if bbox1.shape[0] == 0 or bbox2.shape[0] == 0:
        return torch.zeros(bbox1.shape[0], bbox2.shape[0], device=bbox1.device)
    
    # Must expand the box beforehand, otherwise it may result in overestimated results
    bbox1 = expand_3d_box(bbox1, 0.02)
    bbox2 = expand_3d_box(bbox2, 0.02)
    
    # Convert 8-point boxes to min-max format for IoU calculation
    ...
    inter_size = torch.clamp(inter_max - inter_min, min=0.0)  # Shape: (M, N, 3)
    inter_volume = inter_size[..., 0] * inter_size[..., 1] * inter_size[..., 2]  # Shape: (M, N)
    
    # Compute volumes of individual boxes
    bbox1_size = bbox1_max.squeeze(1) - bbox1_min.squeeze(1)  # Shape: (M, 3)
    bbox2_size = bbox2_max.squeeze(0) - bbox2_min.squeeze(0)  # Shape: (N, 3)
    
    volume1 = bbox1_size[..., 0] * bbox1_size[..., 1] * bbox1_size[..., 2]  # Shape: (M,)
    volume2 = bbox2_size[..., 0] * bbox2_size[..., 1] * bbox2_size[..., 2]  # Shape: (N,)
    
    # Expand volumes for broadcasting
    volume1 = volume1.unsqueeze(1)  # Shape: (M, 1)
    volume2 = volume2.unsqueeze(0)  # Shape: (1, N)
    
    # Compute union volume
    union_volume = volume1 + volume2 - inter_volume  # Shape: (M, N)
