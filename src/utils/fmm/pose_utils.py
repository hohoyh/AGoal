...
# magnum 只在 list2qua() 中用到，而该函数不在 AirSim 主链路上。
# 这里做成延迟导入，避免为了跑主线代码去装 habitat-sim 的图形栈。
def _get_magnum_quaternion():
    try:
        from magnum import Quaternion
    except ImportError as exc:  # pragma: no cover
        raise ImportError(
            "list2qua() 需要 magnum（通常随 habitat-sim 一起安装）。"
            "AirSim 主线不依赖它，可直接 pip install magnum 或忽略该函数。"
        ) from exc
    return Quaternion
...
def list2qua(li):
    Quaternion = _get_magnum_quaternion()
